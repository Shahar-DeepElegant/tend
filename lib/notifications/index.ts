...
const BACKGROUND_REMINDER_TASK = 'friendly-reminder.daily-sync-and-notify';
const OVERDUE_NOTIFICATION_ID = 'friendly-reminder.overdue';
const EVENTS_NOTIFICATION_ID = 'friendly-reminder.events';
const OVERDUE_CHANNEL_ID = 'friendly-reminder-overdue';
const EVENTS_CHANNEL_ID = 'friendly-reminder-events';
...
async function evaluateEventReminders(now: Date) {
  const config = await getConfig();
  const upcoming = await getUpcomingContactEvents(config.contactEventsReminderDays, now.toISOString());
  const eventKeys = upcoming.map((event) => `${event.contactSystemId}|${event.sourceEventId}|${event.nextOccurrenceAt}`);
  ...
  if (newKeys.length === 0) {
    await setNotificationState(LAST_EVENT_HASH_SET_KEY, JSON.stringify([...nextSet]));
    return;
  }

  const contactIds = [...new Set(upcoming.map((event) => event.contactSystemId))];
  ...
  const newEvents = upcoming.filter((event) =>
    newKeys.includes(`${event.contactSystemId}|${event.sourceEventId}|${event.nextOccurrenceAt}`)
  );
  ...
  const title = newEvents.length === 1 ? 'Upcoming contact event' : `Upcoming contact events (${newEvents.length})`;
  await Notifications.scheduleNotificationAsync({
...
async function shouldRunEvaluationToday(now: Date, reminderTime: string, force: boolean) {
...
export async function runDailyReminderEvaluation({
  ...
  const config = await getConfig();
  const now = new Date();
  const shouldRun = await shouldRunEvaluationToday(now, config.reminderNotificationTime, force);
  if (!shouldRun) return false;

  await syncDeviceContactEventsOncePerDay();
  await evaluateOverdueReminders(now);
  await evaluateEventReminders(now);
  await setNotificationState(LAST_DAILY_EVAL_LOCAL_DATE_KEY, toLocalDateKey(now));
